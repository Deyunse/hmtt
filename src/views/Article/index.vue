  ...
  <div class="article-container">
        ...
        <div class="article-detail" v-if="!!article.art_id">
          ...
          <van-cell class="user-info" center :border="false">
            ...
            <!-- <van-button
              ...
              >关注</van-button
            > -->
            <!-- 类似于 :is_foolowed="article.is_followed"  -->
            <FollowUser
    ...
    <div class="article-bottom" v-if="!isLoading && !!article.art_id">
      <van-button
        ...
        >写评论</van-button
      >
      <van-icon name="comment-o" :info="count" color="#777" />
    ...
    <van-popup v-model="addCommentShow" position="bottom"
      ...
      ></AddComment
    ></van-popup>
    <!-- 每次关闭组件销毁 ，每次打开 组件重新创建 mounted就会再一次执行 -->
...
export default {
  ...
  async created () {
    ...
    // 在这里的时候loading关闭了，页面就显示正文，但是必须还要写在nextTick
    // 因为数据更新 试图不能立即更新 写在nextTick 回调中可以 因为newTick回调就是在试图更新完成 才会触发
    let arr1 = []
    this.$nextTick(() => {
      // 获取正文里面所有的图片
      let arr = document.querySelectorAll('.article-content img')
      if (arr.length === 0) return // 防御代码 更加安全
      arr.forEach((img, index) => {
        arr1.push(img.src) // 将图片的地址添加到一个数组中
        img.onclick = function () {
          ImagePreview({
            images: arr1, // 展示图片
            startPosition: index, // 点击就显示当前的图片
            showIndicators: true, // 下方小圆点
            maxZoom: 3
          })
        }
      })
